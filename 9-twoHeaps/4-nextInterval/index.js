...
// minHeapBasedOnEndTime: [ ]
// x: y
// y: z
// z: -1

// minHeapBasedOnStartTime: []
...
// select 1 interval, find from remaining intervals: smallest startTime >= endTime of curInterval
// => minHeap (based on startTime)
...
function findNextIntervals(intervals) {
  const minHeapBasedOnEndTime = new Heap(
    [],
    null,
    (a, b) => b.endTime - a.endTime,
  )
  const minHeapBasedOnStartTime = new Heap(
    [],
    null,
  ...
  for (let i = 0; i < intervals.length; i++) {
    ...
    minHeapBasedOnEndTime.add(new Interval(startTime, endTime, i))
    minHeapBasedOnStartTime.add(new Interval(startTime, endTime, i))
  }

  const output = Array(intervals.length).fill(-1)

  while (minHeapBasedOnEndTime.length) {
    const curInterval = minHeapBasedOnEndTime.pop()

    // GOAL: find the next interval for curInterval
    // minHeapBasedOnStartTime.length === 0;
    if (minHeapBasedOnStartTime.length === 0) break

    while (minHeapBasedOnStartTime.peek().startTime < curInterval.endTime) {
      minHeapBasedOnStartTime.pop()
    }

    // here: minHeapBasedOnStartTime.peek().startTime >= curInterval.endTime
    if (minHeapBasedOnStartTime.length) {
      output[curInterval.index] = minHeapBasedOnStartTime.pop().index
    }
  }
