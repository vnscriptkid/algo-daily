...
function maximizeCapital(capitials, profits, initialCapital, numOfProjects) {
  // (1) we need somehow get all projects that has capital less than myCapital (k-smallest numbers)
  // => use minHeap to have access to project with smallest capital
  const minHeap = new Heap([], null, (p1, p2) => p2.capital - p1.capital)

  for (let i of Array(capitials.length).keys()) {
    minHeap.add(new Project(capitials[i], profits[i]))
  }

  // (2) Amongst those candidate projects, we pick one with largest profit
  // => use maxHeap to access project with largest profit
  const maxHeap = new Heap([], null, (p1, p2) => p1.profit - p2.profit)

  // (3) myProject get updated, back to (1)

  while (numOfProjects > 0) {
    // put projects within my capital in maxHeap
    ...
    initialCapital += maxHeap.pop().profit
    numOfProjects--
  }
